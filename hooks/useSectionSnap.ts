...
export function useSectionSnap(
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      // Lock if:
      // 1. Actively snapping (!releasedRef) — normal snap lock
      // 2. Released but in snap zone — lock early to prevent shiver on upward re-entry
      //    BUT only if user might swipe up (we don't know yet, so lock and let
      //    onTouchEnd unlock if it turns out to be a downward swipe)
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      // If released and swiping DOWN, unlock immediately and bail —
      // this prevents issue #1 from coming back
      if (releasedRef.current && delta > 0) {
        syncTo(window.scrollY);
        unlock();
        return;
      }
