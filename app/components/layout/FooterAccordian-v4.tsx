  ...
  return (
    <div className="lg:hidden bg-white/2">
      {footerV2Data.columns.map((col, i) => {
        ...
        return (
          <div key={i} className="border-b border-white/15 px-5">
            <button
              onClick={() => toggle(i)}
              className="w-full flex items-center justify-between py-[18px] md:py-[20px] text-left container"
            >
            ...
            <AnimatePresence initial={false}>
              {openIndex === i && (
                ...
                >
                  {/* long link lists scroll inside the panel; overscroll-contain + data-lenis-prevent
                      keep that scroll from bubbling to the page (native or Lenis smooth scroll) */}
                  <div
                    data-lenis-prevent
                    className="menu-scrollbar max-h-[30vh] overflow-y-auto overscroll-contain"
                  >
